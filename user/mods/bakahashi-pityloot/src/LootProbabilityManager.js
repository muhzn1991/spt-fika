...
class LootProbabilityManager {
    ...
    getUpdatedLocationLoot(getNewLootProbability, loot, locations, incompleteItemRequirements) {
        ...
        for (const [containerId, container] of Object.entries(loot.staticLoot)) {
            if (missingParts.length > 0 &&
                ...
                missingContainerParts.forEach((itemId) => {
                    config_json_1.debug &&
                        this.logger.info(`Adding gunsmith item to loot tables. container: ${containerId}, itemId: ${itemId}, baseProbability: ${1000}`);
            ...
            if (missingKeys.length > 0 &&
                ...
                missingContainerKeys.forEach((itemId) => {
                    config_json_1.debug &&
                        this.logger.info(`Adding quest key to loot tables. container: ${containerId}, itemId: ${itemId}, baseProbability: ${1000}`);
                    container.itemDistribution.push({
                        relativeProbability: 1000,
                        tpl: itemId,
                    });
