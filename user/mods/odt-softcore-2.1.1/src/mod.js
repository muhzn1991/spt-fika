            ...
            if (config.SecureContainersOptions.Progressive_Containers.enabled) {
                try {
                    ...
                    if (config.SecureContainersOptions.Progressive_Containers.Collector_Quest_Redone.enabled == true) {
                        ...
                        tables.templates.quests["5c51aac186f77432ea65c552"].conditions.AvailableForFinish = tables.templates.quests["5c51aac186f77432ea65c552"].conditions.AvailableForFinish.filter((x) => x.conditionType != "Level");
                        // Start condition
        ...
        if (config.HideoutOptions.enabled) {
            ...
            for (const prod in tables.hideout.production) {
                ...
                if ((endProduct == "5d1b376e86f774252519444e" || endProduct == "5d1b33a686f7742523398398") &&
                    ...
                    tables.hideout.production[prod].productionTime = Math.round(productionTime / config.HideoutOptions.Faster_Moonshine_and_Purified_Water_Production.Base_Moonshine_And_Water_Time_Multiplier);
                }
                else if (endProduct == "59faff1d86f7746c51718c9c" && config.HideoutOptions.Faster_Bitcoin_Farming.enabled == true) {
                    // bitcoins
                    tables.hideout.production[prod].productionTime = Math.round(productionTime / config.HideoutOptions.Faster_Bitcoin_Farming.Base_Bitcoin_Time_Multiplier);
            ...
            if (config.HideoutOptions.Faster_Hideout_Construction.enabled) {
                ...
                for (const area in tables.hideout.areas) {
                    for (const stage in tables.hideout.areas[area].stages) {
                        tables.hideout.areas[area].stages[stage].constructionTime = Math.round(tables.hideout.areas[area].stages[stage].constructionTime / config.HideoutOptions.Faster_Hideout_Construction.Hideout_Construction_Time_Multiplier);
                    }
                }
            }
                ...
                if (config.EconomyOptions.Price_Rebalance.enabled) {
                    try {
                        for (const handbookItem in tables.templates.handbook.Items) {
                            ...
                            if ((!fleaListingsWhitelist.includes(itemInHandbook.ParentId) && config.EconomyOptions.Pacifist_FleaMarket.enabled) ||
        ...
        if (config.TraderChanges.enabled) {
            ...
            if (config.TraderChanges.Pacifist_Fence.enabled == true) {
                try {
                    ...
                    fenceBlacklist.push(...BSGblacklist_1.BSGblacklist, ...fleaBarterRequestBlacklist
